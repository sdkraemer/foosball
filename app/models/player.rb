...
class Player < ActiveRecord::Base
	...
	def goals_per_stick
		total_positions = Position.where(:positions => {player_id: self.id}).joins(:team => :game).merge(Game.completed).count
	...
	def player_wins
		@player_wins ||= my_teams.merge(Team.winner).count
	end

	def player_losses
		@player_losses ||= my_teams.merge(Team.loser).count
	end
	...
	def my_teams
		return @my_teams if defined? @my_teams
		@my_teams ||= Team.uniq.joins(:positions).where(:positions => {:player_id => self.id}).joins(:game).merge(Game.completed)
	end
	...
	def plus_minus
		...
		opposing_teams = Team.joins(:game).where(:games => {id: my_completed_games}).where.not(:teams => {id: my_teams}).uniq

		my_teams_scored_goals = Goal.scored_goal.joins(:position => :team).where(:teams => {id: my_teams}).count
		my_teams_own_goals = Goal.own_goal.joins(:position => :team).where(:teams => {id: my_teams}).count
